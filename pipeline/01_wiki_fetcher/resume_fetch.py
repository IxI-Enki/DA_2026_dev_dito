...
class ResumeFetcher:
    ...
    def run_resume(self, retry_pages: bool = True, retry_media: bool = True) -> Dict:
        ...
        if retry_media and failed_media:
            ...
            for i, media_id in enumerate(failed_media, 1):
                ...
                if media_id.startswith("http") or "%" in media_id:
                    self.log(
                        f"  [{i}/{len(failed_media)}] {media_id[:50]} - external URL, skipping"
                    )
                    continue
        ...
        self.log("=" * 60)
        self.log(
            f"Pages: {self.resume_stats['pages_recovered']}/{self.resume_stats['pages_retried']} recovered"
        )
        self.log(
            f"Media: {self.resume_stats['media_recovered']}/{self.resume_stats['media_retried']} recovered"
        )
...
def main():
    ...
    parser.add_argument(
        "output_dir", help="Output directory name to resume (e.g. fetched_at_20251221_203927)"
    )
    ...
    try:
        stats = fetcher.run_resume(retry_pages=not args.no_pages, retry_media=not args.no_media)

        # Exit code based on success
        stats["pages_recovered"] + stats["media_recovered"]
