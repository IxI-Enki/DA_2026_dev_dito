...
from manifest import FetchManifest, MediaEntry, PageEntry
...
class ExtendedWikiFetcher:
    ...
    def __init__(
        ...
        self.manifest = FetchManifest(
            wiki_url=API_BASE_URL, fetch_id=output_dir or self._generate_manifest_id()
        )
        ...
        try:
            ...
            for page in wiki_pages:
                ...
                if page_id and page_id not in page_dict:
                    page_dict[page_id] = page
            self.log(
                f"    Found: {len(wiki_pages)} pages (+{len(wiki_pages) - len([p for p in wiki_pages if p.get('id') in page_dict])} new)"
            )
        ...
        for ns in namespaces_to_try:
            try:
                response = self.client.call(
                    "wiki.getPagelist",
                    {"namespace": ns, "options": {"depth": 0}},  # Unlimited depth
                )
                for page in response.get("result", []):
                    page_id = page.get("id", "")
    ...
    def _filter_pages(self, pages: List[Dict]) -> List[Dict]:
        ...
        if (
            not self.config.filter.include_namespaces
            and not self.config.filter.exclude_namespaces
            and not self.config.filter.exclude_pages
        ):
            return pages
    ...
    def extract_namespaces(self, pages: List[Dict]) -> Dict[str, List[str]]:
        ...
        for ns, pages_list in namespaces.items():
            has_start = f"{ns}:start" in pages_list or (
                ns == "root" and "start" in [p for p in pages_list]
            )
        ...
        namespace_tree = {
            ...
            "namespaces": {
                ns: {
                    "page_count": len(pages_list),
                    "has_start_page": self.stats["namespaces"]["by_namespace"][ns][
                        "has_start_page"
                    ],
    ...
    def _init_media_cache(self) -> None:
        ...
        if "cache" not in self.stats["media"]:
            self.stats["media"]["cache"] = {"hits": 0, "misses": 0, "bytes_saved": 0}
    ...
    def download_media_files(self, media_list: List[Dict]) -> None:
        ...
        (
            self.config.media.max_file_size_mb * 1024 * 1024
            if self.config.media.max_file_size_mb > 0
            else float("inf")
        )
        ...
        for i, media in enumerate(media_list, 1):
            ...
            if self.use_cache and self.media_cache:
                copy_result = self.media_cache.copy_from_cache(
                    media_id, file_path, verify=self.config.cache.verify_on_copy
                )
                if copy_result:
                    # Cache hit!
        ...
        if self.use_cache:
            self.log(
                f"    - From cache: {cached_count} (saved {format_bytes(self.stats['media']['cache']['bytes_saved'])})"
            )
        ...
        try:
            ...
            if self.config.content.fetch_acl:
                ...
                if namespace not in self.stats["acl_summary"]["by_namespace"]:
                    self.stats["acl_summary"]["by_namespace"][namespace] = {
                        "pages": 0,
                        "permission_levels": {},
                    }
            ...
            if self.config.content.fetch_backlinks:
                ...
                for _linking_page in backlinks:
            ...
            if self.config.content.fetch_html:
                ...
                if page_html:
                    ...
                    if self.config.content.fetch_links:
                        ...
                        total_links = links_data["summary"].get(
                            "total_links", internal_count + external_count + media_count
                        )
                        ...
                        self._page_link_counts.append(
                            {"id": page_id, "total": total_links, "internal": internal_count}
                        )
    ...
    def _finalize_statistics(self):
        ...
        if self.stats["pages"]["successful"] > 0:
            ...
            self.stats["pages"]["avg_html_size"] = (
                ...
                if self.stats["pages"]["with_html"] > 0
                else 0
            )
        ...
        if self.stats["pages"]["with_links"] > 0:
            ...
            self.stats["links"]["avg_links_per_page"] = (
                total_links / self.stats["pages"]["with_links"]
            )

        # Most linked pages (top 20)
        sorted_targets = sorted(
            self._internal_link_targets.items(), key=lambda x: x[1], reverse=True
        )[:20]
        self.stats["links"]["most_linked_pages"] = dict(sorted_targets)

        # External domains (top 20)
        sorted_domains = sorted(self._external_domains.items(), key=lambda x: x[1], reverse=True)[
            :20
        ]
        self.stats["links"]["external_domains"] = dict(sorted_domains)

        # Pages with most links (top 10)
        sorted_pages = sorted(self._page_link_counts, key=lambda x: x["total"], reverse=True)[:10]
    ...
    def _build_media_usage_index(self) -> None:
        ...
        most_referenced = sorted(
            [
                (mid, info["reference_count"])
                for mid, info in media_usage.items()
        ...
        index = {
            ...
            "media_with_references": sum(
                1 for u in media_usage.values() if u["reference_count"] > 0
            ),
        ...
        self.log(
            f"    Total media: {len(media_usage)}, with references: {index['media_with_references']}, orphans: {len(orphan_media)}"
        )
        ...
        try:
            for info_file in self.paths["page_metadata"].glob("*_info.json"):
                ...
                try:
                    ...
                    modified = data.get(
                        "revision", data.get("modified", data.get("lastModified", 0))
                    )
                    # API returns 'id', fallback to 'name' or filename
                    page_id = data.get(
                        "id",
                        data.get("name", info_file.stem.replace("_info", "").replace("_", ":")),
                    )
                    if modified:
    ...
    def _analyze_missing_media(self) -> List[Dict[str, str]]:
        ...
        for media_id in missing_list:
            ...
            analyzed.append({"media_id": media_id, "status": status, "reason": reason})

        return analyzed

    def _get_deepest_pages(self, limit: int = 20) -> List[Dict[str, Any]]:
        """Get pages with deepest namespace paths"""
        pages_with_depth = [{"id": p["id"], "depth": p["id"].count(":")} for p in self._page_sizes]
    ...
    def _save_analysis_report(self):
        ...
        for ns, data in sorted(
            self.stats["namespaces"]["by_namespace"].items(),
            ...
            report_lines.append(
                f"  {ns}: {data['page_count']} pages, {media_count} media {has_start}"
            )
        ...
        if missing_analyzed:
            ...
            for item in missing_analyzed[:25]:
                media_short = (
                    item["media_id"][:43] if len(item["media_id"]) > 43 else item["media_id"]
                )
                report_lines.append(
    ...
    def print_summary(self):
        ...
        self.log(
            f"   Largest: {self.stats['pages']['largest_page']['id']} ({format_bytes(self.stats['pages']['largest_page']['size'])})"
        )
...
def main():
    ...
    parser.add_argument(
        "--no-manifest", action="store_true", help="Skip manifest generation (for testing)"
    )
    parser.add_argument(
        "--show-manifest", type=str, metavar="PATH", help="Show manifest summary and exit"
    )
    parser.add_argument(
        "--verify-manifest", type=str, metavar="PATH", help="Verify manifest integrity and exit"
    )
    parser.add_argument(
        "--compare-manifests",
        nargs=2,
