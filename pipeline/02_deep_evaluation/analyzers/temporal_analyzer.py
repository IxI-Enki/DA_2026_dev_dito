...
class TemporalAnalyzer:
    ...
    def _analyze_page(self, page_data: Dict[str, Any]) -> PageTemporalInfo:
        ...
        rag_weight = self._calculate_rag_weight(
            freshness_category, is_archived, has_outdated_refs
        )
    ...
    def _parse_last_modified(self, metadata: Dict) -> Optional[datetime]:
        ...
        for field in ['last_modified', 'lastModified', 'modified', 'date']:
    ...
    def _calculate_freshness(
        ...
        if last_modified:
            ...
            if age_days <= self.current_days:
                ...
                score = 1.0 - (age_days - self.current_days) / (self.recent_days - self.current_days) * 0.3
                return 'recent', score
            elif age_days <= self.outdated_days:
                # Linear interpolation between 0.7 and 0.4
                score = 0.7 - (age_days - self.recent_days) / (self.outdated_days - self.recent_days) * 0.3
    ...
    def _update_stats(self, info: PageTemporalInfo):
        ...
        self.result.freshness_distribution[cat] = \
            self.result.freshness_distribution.get(cat, 0) + 1
        ...
        for year in info.year_references:
            self.result.year_distribution[year] = \
                self.result.year_distribution.get(year, 0) + 1
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'year_distribution': dict(sorted(
                self.result.year_distribution.items(),
                key=lambda x: x[0],
                reverse=True
            )),
            ...
            'archived_pages': [
                ...
                for p in self.result.pages if p.is_archived
            ],
            'outdated_pages': [
                {
                    'page_id': p.page_id,
            ...
            'time_sensitive_pages': [
                p.page_id for p in self.result.pages if p.is_time_sensitive
            ]
        }
