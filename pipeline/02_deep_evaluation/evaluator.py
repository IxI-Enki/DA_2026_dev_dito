...
class ContentEvaluator:
    ...
    def _evaluate_page(self, page_id: str) -> PageEvaluation:
        ...
        if evaluation.char_count < self.MIN_CONTENT_LENGTH:
            ...
            evaluation.flags.append({"type": "stub", "message": f"Page is a stub ({evaluation.char_count} chars)"})
        
        # Check for template-only
        if self._is_template_only(content):
            evaluation.is_template_only = True
            evaluation.flags.append({"type": "template", "message": "Page contains only template markup"})
        
        # Count structure elements
        evaluation.heading_count = content.count("====") + content.count("===") + content.count("==")
    ...
    def _score_content(self, content: str, evaluation: PageEvaluation) -> float:
        ...
        has_variety = (
            evaluation.heading_count > 0 or
            evaluation.code_block_count > 0 or
            evaluation.table_count > 0
        )
        if has_variety:
            score += 0.3
        elif evaluation.char_count > 500:
