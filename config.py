...
if __name__ == "__main__":
    ...
    print(
        f"  TOKEN:         {'*****' + SOURCE_WIKI_TOKEN[-4:] if SOURCE_WIKI_TOKEN else '(nicht gesetzt)'}"
    )
    ...
    print(f"\n[Pipeline]")
    print(
        f"  Fetcher:       timeout={FETCHER_TIMEOUT}s, retries={FETCHER_MAX_RETRIES}, delay={FETCHER_DELAY}s"
    )
