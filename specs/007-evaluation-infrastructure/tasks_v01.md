...
- [x] T029 Write integration test `evaluation/tests/test_integration.py` — end-to-end with local Qdrant (embed small corpus, query, verify metrics are non-zero)
  - Implemented: two tests (`test_qdrant_connection`, `test_e2e_qdrant_embed_query_metrics`); both skip when Qdrant or Ollama unavailable. Run with Stack-G Qdrant (port 18334) and Ollama (bge-m3) to execute. Verified: 2 passed (uses `query_points`, `check_compatibility=False`).
...
| `test_integration.py`      |      2 | Qdrant connection smoke; e2e embed+query+metrics (skip if Qdrant/Ollama down)          |
| **Total**                  | **58** | **56 unit + 2 integration** — integration pass when Stack-G Qdrant (18334) + Ollama up |
...
- **Config**: Scripts read `config/env.yaml` for Qdrant (`SERVICES.qdrant` port 18334, `qdrant_test` port 18336) and Wiki (`SOURCE_WIKI.api.url`); no import from `pipeline/01_wiki_fetcher/config.py` (avoids side effects).
...
- **Tests**: `pytest evaluation/tests/` — 56 unit + 2 integration; all 58 pass when Stack-G Qdrant (host 18334) and Ollama (bge-m3) are running. Integration tests skip when services unavailable.
- **Integration run verified**: 2026-02-13 — `pytest evaluation/tests/test_integration.py -v` → 2 passed (test_qdrant_connection, test_e2e_qdrant_embed_query_metrics) with `docker compose -p stack-g-devdito up -d` (qdrant + orchestrator).
